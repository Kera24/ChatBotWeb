...
def test_alembic_upgrade_through_head_uses_wide_version_table(tmp_path: Path) -> None:
    ...
    assert current_revision == "0012_widget_knowledge_preview_installation"
...
def test_revision_history_is_linear_and_identifiers_are_preserved() -> None:
    ...
    assert script.get_heads() == ["0012_widget_knowledge_preview_installation"]
    assert required_version_length(script) >= len("0012_widget_knowledge_preview_installation")
    assert [revision.revision for revision in revisions][0] == "0012_widget_knowledge_preview_installation"
    for revision in revisions:
        down_revision = revision.down_revision
        assert not isinstance(down_revision, tuple)
    assert {revision.revision for revision in revisions} >= {
        "0011_widget_embed_preferences",
        "0012_widget_knowledge_preview_installation",
    }
