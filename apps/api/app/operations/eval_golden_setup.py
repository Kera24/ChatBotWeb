...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--database-url", default=None, help=f"Defaults to a local SQLite file at {_DEFAULT_DB_PATH}.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    database_url = args.database_url or f"sqlite:///{_DEFAULT_DB_PATH}"
    ...
    with session_factory() as db:
        existing_organisation = db.execute(select(Organisation).where(Organisation.slug == _GOLDEN_ORG_SLUG)).scalar_one_or_none()
        if existing_organisation is not None:
            ...
            organisation = Organisation(name="Golden Evaluation Organisation", slug=_GOLDEN_ORG_SLUG, status="active", plan_key="starter")
            workspace = Workspace(organisation=organisation, name="Golden Evaluation Workspace", slug="golden-eval-workspace", status="active", default_language="en")
        ...
        print(
            ...
            f"--organisation {existing_organisation.id} --workspace {workspace.id} --format json"
        )
