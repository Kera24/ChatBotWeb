...
| `scripts/release-gate.mjs`, `.env.production.example` | `node scripts/release-gate.mjs --skip-verified-suites --migration-check-env-file .env.production.example` (exercises the config-shape, Compose config, and real-migration gates without re-running suites `npm run verify` already covers) **and**, if `--env-file`/mandatory-var behaviour changed, `node scripts/release-gate.mjs --env-file .env.production.example` once to confirm `required_env_vars_present`/`required_env_vars_not_placeholder` still behave as expected (the latter is *supposed* to fail against the example file - see the script's own comments) |
| `.github/workflows/**` | Do not modify without explicit instruction (see `docs/architecture/deployment.md`); if explicitly asked: validate YAML syntax locally (e.g. `python -c "import yaml; yaml.safe_load(open(path))"`), and run the actual command each new/changed step invokes locally too (`npm run verify`, `npm run vps:release:gate -- --skip-verified-suites --migration-check-env-file .env.production.example`, etc.) — this repo's CI steps are themselves just npm/node scripts, not GitHub-Actions-specific logic, so most of a workflow change's real behaviour IS locally verifiable. Only genuinely CI-only concerns (runner-specific environment, `workflow_dispatch` inputs, secrets that only exist in GitHub) are not |
| `infrastructure/azure/**` | Do not modify without explicit instruction (see `docs/architecture/deployment.md`); if explicitly asked, there is no local equivalent to a real Azure environment — describe what you changed precisely and flag that it can only be fully validated by CI/a real Azure environment |
