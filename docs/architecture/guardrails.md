...
## Evidence Sufficiency V1/V2

`app.ai.guardrails.evidence_sufficiency` ships two verifier implementations behind one injectable `EvidenceVerifier` interface — `EvidenceSufficiencyV1Verifier` (the original per-chunk requested-fact/proximity/conflict logic, completely unmodified) and `EvidenceSufficiencyV2Verifier` (additive fixes to three proven false-rejection mechanisms: case-sensitive value extraction, first-match-in-window value selection instead of nearest-to-anchor, and a stricter keyword-hit bar applied only when arbitrating a conflicting-evidence verdict — see the module's V2 section docstring for the full, evidence-cited rationale and `docs/adr/0034-promote-evidence-sufficiency-v2.md` for the real-embedding bake-off that promoted it). `settings.EVIDENCE_VERIFIER_VERSION` (`"v1"`/`"v2"`, default `"v2"`) selects which one `RAGOrchestratorDependencies.evidence_verifier` and `EvaluationRunOptions.evidence_verifier_override` resolve to — mirrors `RETRIEVAL_STRATEGY`/`RERANKER_PROVIDER`/`QUERY_TRANSFORMER_PROVIDER`'s exact config-driven-with-one-line-rollback pattern. `app.operations.eval_evidence_sufficiency_failure_analysis` and `app.operations.eval_evidence_sufficiency_bakeoff` are the case-level failure-taxonomy and controlled-comparison tooling this decision was based on.
